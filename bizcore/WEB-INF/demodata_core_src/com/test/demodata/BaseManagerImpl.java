...
public class BaseManagerImpl implements AccessControledService, BeanNameAware {
	...
	// To enable the class to get the bean name, then all managers know who they
	// are,
	...
	public void ensureRelationInGraph(UserContext userContext, BaseEntity sourceEntity) {
		...
		GraphService graphService = ((DemodataUserContextImpl) userContext).getGraphService();
		if (graphService instanceof GraphServiceImpl) {
			((GraphServiceImpl) graphService).addNode(getSystemInternalName(), sourceEntity.getInternalType(),
					sourceEntity.getId());
			...
			for (BaseEntity entity : entityList) {
				((GraphServiceImpl) graphService).addEdge(getSystemInternalName(),
						sourceEntity.getInternalType() + "_" + entity.getInternalType(), sourceEntity.getInternalType(),
						entity.getInternalType(), sourceEntity.getId(), entity.getId());
			}
		}
	...
	public void sendCreationEvent(UserContext userContext, BaseEntity sourceEntity) {
		EventService eventService = ((DemodataUserContextImpl) userContext).getEventService();
	...
	public void deleteRelationInGraph(UserContext userContext, BaseEntity sourceEntity) {
		...
		GraphService graphService = ((DemodataUserContextImpl) userContext).getGraphService();
		if (graphService instanceof GraphServiceImpl) {
			((GraphServiceImpl) graphService).deleteNode(getSystemInternalName(), sourceEntity.getInternalType(),
					sourceEntity.getId());
		}
	...
	public Object checkAccess(BaseUserContext baseUserContext, String methodName, Object[] parameters)
		...
		String message = "你以持有对" + sourceType + "(" + sourceId + ")的 '" + permission + "' 权限被禁止以 '" + operation
				+ "' 访问: " + targetObjectType + "(" + targetId + ")";
		try {
	...
	protected boolean hasIndirectRight(UserContext userContext, String permission, String requiredPermission,
	...
	protected String getUserKey(UserContext userContext) {
		return "demodata:user:" + userContext.tokenId();
	}
	...
	protected Object invokeWithOneString(BaseUserContext baseUserContext, String beanName, String methodName,
			String classBaseName, String parameter) throws Exception {
		UserContext userContext = (UserContext) baseUserContext;
		Object target = userContext.getBean(beanName);
		Method method = target.getClass().getMethod(methodName,
				(Class<?>[]) new Class<?>[] { DemodataUserContext.class, String.class });
	...
	protected void packMessage(List<Message> messageList, String subject, String propertykey, Object[] parameters,
	...
	protected void checkStringLengthRange(String value, int minLength, int maxLength, String propertyKey,
			List<Message> messageList) {
		...
		if (value == null) {
			...
			packMessage(messageList, "STRING_NOT_ALLOW_TO_BE_NULL", propertyKey,
		...
		if (minLength == maxLength) {
			...
			// errorMsg.setBody("您输入的"+propertyKey+":'"+value+"'长度有误, 该值长度为 "+
			// value.length()+", 系统预期是固定长度为"+minLength + "之间的文本");
			packMessage(messageList, "STRING_NOT_FIXED_LENGTH", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength, value.length() },
					"您输入的 '" + propertyKey + "' 的值'" + value + "'长度不对, 该值长度为 " + value.length() + ", 系统预期是固定长度为 "
							+ minLength + " 文本.");
			return;
		}
		if (value.length() > maxLength) {
			packMessage(messageList, "STRING_TOO_LONG", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength, value.length() }, "您输入的 '" + propertyKey
							+ "' 的值'" + value + "'长度太长, 该值长度为 " + value.length() + ", 系统预期最多 " + maxLength + " 个字符.");
			return;
		}
		if (value.length() < minLength) {

			packMessage(messageList, "STRING_TOO_SHORT", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength, value.length() }, "您输入的 '" + propertyKey
							+ "' 的值'" + value + "'长度太短, 该值长度为 " + value.length() + ", 系统预期是最短 " + minLength + " 个字符.");
			return;
		}
	...
	protected void checkChinaMobilePhone(String value, int minLength, int maxLength, String propertyKey,
			List<Message> messageList) {
		...
		if (value == null) {
			...
			packMessage(messageList, "CHINA_MOBILE_NOT_ALLOW_TO_BE_NULL", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength },
		...
		if (!integerValueInClosedRange(value.length(), 11, 11)) {
			packMessage(messageList, "CHINA_MOBILE_NOT_FIXED_LENGTH", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength, value.length() },
					"您输入的 '" + propertyKey + "' 的值 '" + value + "' 长度不对, 该值长度为 " + value.length() + ", 系统预期是固定长度为 "
							+ minLength + "，而且全部为数字的手机号.");
		...
		if (!prefixedWith(value, prefixes)) {
			...
			packMessage(messageList, "CHINA_MOBILE_FORMAT_ISSUE", propertyKey,
					new Object[] { propertyKey, value, minLength, maxLength, value.length() },
		...
		for (char ch : digits) {
			...
			packMessage(messageList, "CHINA_MOBILE_CONTAIN_INVALID_CHAR", propertyKey,
	...
	protected void checkDateRange(Date value, Date minDate, Date maxDate, String propertyKey,
		...
		if (value == null) {
			...
			packMessage(messageList, "DATE_NOT_ALLOWED_TO_BE_NULL", propertyKey,
					new Object[] { propertyKey, value, minDate, maxDate }, "您输入的 '" + propertyKey + "' 为空，系统预期为一个日期.");
			return;
		}
		Format formatter = new SimpleDateFormat("yyyy-MM-dd");
		if (minDate.after(value)) {
			packMessage(messageList, "DATE_BEFORE_START", propertyKey,
		...
		if (maxDate.before(value)) {
			packMessage(messageList, "DATE_AFTER_END", propertyKey,
					new Object[] { propertyKey, value, minDate, maxDate },
	...
	protected void checkTimeRange(Date value, Date minDate, Date maxDate, String propertyKey,
		...
		if (value == null) {
			...
			packMessage(messageList, "DATE_NOT_ALLOWED_TO_BE_NULL", propertyKey,
					new Object[] { propertyKey, value, minDate, maxDate }, "您输入的 '" + propertyKey + "' 为空，系统预期为一个时间.");
			return;
		}
	...
	protected void checkEmail(String value, int min, int max, String propertyKey, List<Message> messageList) {
		checkStringLengthRange(value, 5, 256, propertyKey, messageList);
		/*
		 * The maximum length is specified in RFC 5321: "The maximum total length of a
	...
	protected void checkPassword(String value, int min, int max, String propertyKey, List<Message> messageList) {
	...
	protected void checkDoubleRange(double value, double min, double max, String propertyKey,
			List<Message> messageList) {
		if (value > max) {
			packMessage(messageList, "NUMBER_GREATER_THAN_MAX", propertyKey,
	...
	protected void checkIntegerRange(int value, int min, int max, String propertyKey, List<Message> messageList) {
		if (value > max) {
			packMessage(messageList, "INTEGER_GREATER_THAN_MAX", propertyKey,
					new Object[] { propertyKey, value, min, max },
					"您输入的" + propertyKey + "在比允许的最大值" + max + "还要大，请修正。");
		...
		if (value < min) {
			packMessage(messageList, "INTEGER_LESS_THAN_MIN", propertyKey,
					new Object[] { propertyKey, value, min, max },
					"您输入的" + propertyKey + "在比允许的最小值" + min + "还要小，请修正。");
	...
	protected <T extends BaseEntity> void addAction(BaseUserContext userContext, BaseEntity baseEntity,
			Map<String, Object> tokens, String localeKey, String key, String path, String group, String level) {
	...
	protected Action createAction(String localeKey, String key, String path, String actionGroup, String actionLevel) {
		...
		return new Action().withLocaleKey(localeKey).withActionKey(key).withManagerBeanName(this.getBeanName())
				.withActionPath(path).withActionGroup(actionGroup).withActionLevel(actionLevel);
	...
	protected void checkMoneyAmount(BigDecimal value, double min, double max, String propertyKey,
			List<Message> messageList) {
	...
	protected void checkBigDecimalRange(BigDecimal value, double min, double max, String propertyKey,
			List<Message> messageList) {
	...
	protected void checkTaxpayerid(String value, int min, int max, String propertyKey, List<Message> messageList) {
	...
	protected void applyAccessList(UserContext userContext, Object targetObject) {
		...
		for (SmartList<?> list : allLists) {
			...
			if (!accessInfo.getReadPermission()) {
				userContext.log(list.getListInternalName() + " is " + accessInfo.getReadPermission());
