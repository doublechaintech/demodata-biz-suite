<%@ page import='java.util.*,com.test.demodata.*,com.test.demodata.platform.Platform'%>
...
<div class="row">
	...
	<% Platform result = (Platform)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#imageList" class="disabled"> ${userContext.localeMap['image']}</a></li>
...
<sky:include page="com/test/demodata/platform/Platform$Summary.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["imageList"] or ignoreListAccessControl}'>
		<c:set var="imageList" value="${result.imageList}" scope="request"/>
		<c:set var="imageListName" value="imageList" scope="request"/>
		<div id="imageList" class="tab-pane fade sublist" refer-name="platform">
			<sky:include page="com/test/demodata/image/Image$List.jsp"
					referName="platform"/>
		</div>
	</c:if>
