...
class PlatformBizApp extends React.PureComponent {
  ...
  getImageSearch = () => {
    const {ImageSearch} = GlobalComponents;
    const userContext = null
    return connect(state => ({
      rule: state.rule,
      name: "Image",
      role: "image",
      data: state._platform.imageList,
      metaInfo: state._platform.imageListMetaInfo,
      count: state._platform.imageCount,
      currentPage: state._platform.imageCurrentPageNumber,
      searchFormParameters: state._platform.imageSearchFormParameters,
      ...
      listName: 'imageList', ref:state._platform, 
      listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(ImageSearch)
  }
  getImageCreateForm = () => {
   	const {ImageCreateForm} = GlobalComponents;
   	const userContext = null
    return connect(state => ({
      rule: state.rule,
      role: "image",
      ...
      searchFormParameters: state._platform.imageSearchFormParameters,
      loading: state._platform.loading,
      owner: { type: '_platform', id: state._platform.id, referenceName: 'platform', listName: 'imageList', ref:state._platform, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(ImageCreateForm)
  }
  
  getImageUpdateForm = () => {
    const userContext = null
  	const {ImageUpdateForm} = GlobalComponents;
    return connect(state => ({
      selectedRows: state._platform.selectedRows,
      role: "image",
      currentUpdateIndex: state._platform.currentUpdateIndex,
      owner: { type: '_platform', id: state._platform.id, listName: 'imageList', ref:state._platform, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(ImageUpdateForm)
  }
  ...
  buildRouters = () =>{
  	...
  	{path:"/platform/:id/list/imageList", component: this.getImageSearch()},
  	{path:"/platform/:id/list/imageCreateForm", component: this.getImageCreateForm()},
  	{path:"/platform/:id/list/imageUpdateForm", component: this.getImageUpdateForm()},
