...
export default {
  ...
  effects: {
    ...
    *addImage({ payload }, { call, put }) {
      ...
      const data = yield call(PlatformService.addImage, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/platform/${id}/list/\ImageList/Image+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateImage({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.updateImage, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/platform/${id}/list/\ImageList/Image列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    *gotoNextImageUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeImageList({ payload }, { call, put }) {
     const userContext = null
      const {PlatformService} = GlobalComponents; 
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.removeImageList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
