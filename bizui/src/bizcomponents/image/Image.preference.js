...
import styles from './Image.preference.less'
...
const internalSettingListOf = (image) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (image) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {ImageService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = ImageService[candidateServiceName] 
...
const executeTrans = (image,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_image/doJob",payload: payload})
...
const buildTransferModal = (image,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(image,targetComponent)}
...
const internalRenderExtraHeader = (image) =>{
	return null
}
const internalRenderExtraFooter = (image) =>{
	return null
}
...
const internalSummaryOf = (image,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{image.id}</Description> 
<Description term="Name">{image.name}</Description> 
<Description term="Create Time">{ moment(image.createTime).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(image,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = image => {
  const {ImageBase} = GlobalComponents
  return <PermissionSetting targetObject={image}  targetObjectMeta={ImageBase}/>
...
class ImagePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.image
    const cardsData = {cardsName:"Image",cardsFor: "image",cardsSource: this.props.image,
  		subItems: [
