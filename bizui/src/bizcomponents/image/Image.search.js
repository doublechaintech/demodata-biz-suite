...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{imageList:selectedRows}} owner={owner}  
...
class ImageSearch extends PureComponent {
  ...
  render(){
    ...
    const {ImageTable} = GlobalComponents;
    const {ImageSearchForm} = GlobalComponents;
    const {ImageModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        ...
        {showDeletionDialog(this,ImageModalTable,"imageIds")}
...
export default Form.create()(ImageSearch)
