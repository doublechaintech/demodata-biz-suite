...
import styles from './Image.table.less'
...
class ImageTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ImageBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/image/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
